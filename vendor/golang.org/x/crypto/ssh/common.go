...
type Config struct {
	...
	// ChannelWindowSize sets the initial flow-control window advertised for new
	// SSH channels. If zero, a default matching OpenSSH is used.
	ChannelWindowSize uint32

	// ChannelMaxPacketSize sets the maximum channel payload size advertised for
	// new SSH channels. If zero, a default matching OpenSSH is used.
...
func (c *Config) SetDefaults() {
	...
	if c.ChannelMaxPacketSize == 0 {
		c.ChannelMaxPacketSize = channelMaxPacket
	} else if c.ChannelMaxPacketSize < minPacketLength {
		c.ChannelMaxPacketSize = minPacketLength
	} else if c.ChannelMaxPacketSize > maxSupportedChannelMaxPacket {
		c.ChannelMaxPacketSize = maxSupportedChannelMaxPacket
	}

	if c.ChannelWindowSize == 0 {
		c.ChannelWindowSize = channelWindowSize
	} else if c.ChannelWindowSize < c.ChannelMaxPacketSize {
		c.ChannelWindowSize = c.ChannelMaxPacketSize
	}
